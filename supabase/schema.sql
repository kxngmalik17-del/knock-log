...
-- 12. Events Table (Anti-Gravity Outbox Destination)
...
create policy "events_select_own" on public.events for select using (auth.uid() = rep_id);
create policy "events_insert_own" on public.events for insert with check (auth.uid() = rep_id);

-- 13. Trigger to project events into legacy tables
